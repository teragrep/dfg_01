...
- name: Update from release
  ...
  with:
    ...
    max_attempts: 1
    repo_baseurl: https://raw.githubusercontent.com/${{ inputs.to_repository }}/refs/heads/${{ inputs.to_branch }}/${{ inputs.to_path }}
    repo_name: teragrep-central-releases
----
...
|`repo_baseurl`
|The `baseurl=` used in the `.repo` file. Supports `inputs` variables.
