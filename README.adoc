...
- name: Update from release
  ...
  with:
    ...
    max_attempts: 1
    repo_baseurl: https://custom.repo.example.com/
...
|`repo_baseurl`
|The baseurl used in the `.repo` file. Note that it is evaluted to the default value if left empty, and that it can't contain variables if explicitly set.
